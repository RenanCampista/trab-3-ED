...
void analyze_word(Vector *words, HashTable *index, char full_file_name[200]) {
    ...
    for (int i = 0; i < vector_size(words); i++) {
        ...
        if (hash_table_contains_key(index, w)) {
            Word *word = (Word *)_hash_pair_get_item(hash_table_get(index, w));
            ...
            hash_table_insert(index, word_get_name(new_word), new_word);
        }
    }
}

void read_file_words(char full_file_name[200], HashTable *index) {
...
HashTable *build_index(string dir_in) {
    char full_dir[100];
...
void save_index(HashTable *index, string dir_out) {
    FILE *file = fopen(dir_out, "w");
    if (file == NULL)
        exit(printf("Error: save_index: could not open file %s.\n\n", dir_out));

    int n_words = hash_table_size(index);
    fprintf(file, "%d\n", n_words);
    
    Vector *pairs = hash_table_all_pairs(index);
    for (int i = 0; i < vector_size(pairs); i++) {
        Pair *p = (Pair *)vector_get(pairs, i);
        Word *word = (Word *)_hash_pair_get_item(p);
        word_print(word, file);
    }
...
int main(int argc, char *argv[]) {
    ...
    HashTable *index = build_index(dir_in);
