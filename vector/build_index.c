...
void analyze_word(Vector *words, Vector *index, char full_file_name[200]) {
    // Para cada palavra do arquivo
    for (int i = 0; i < vector_size(words); i++) {
        string w = (string)vector_get(words, i);
        Word *wr = vector_search(index, w, word_compare);

        // Se a palavra já existe no índice
        if (wr != NULL) {
            Document *doc = word_get_doc_from_file_name(wr, full_file_name);
            ...
            vector_push_back(index, new_word);
        }
    }
}

void read_file_words(char full_file_name[200], Vector *index) {
...
Vector *build_index(string dir_in) {
    char full_dir[100];
...
int main(int argc, char *argv[]) {
    ...
    Vector *index = build_index(dir_in);
